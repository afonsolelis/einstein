...
function render(s){
  ...
  renderKey=key;
  // O caso e as alternativas seguem projetados na discussão e na rodada 2; o resto entra abaixo.
  let html=caseView(s);
  if(s.phase==='discussion'||s.phase==='round2')html+=`${s.phase==='discussion'?'<div class="prompt"><strong>Pergunta para circular pela sala</strong><p>Qual hipótese precisa ser verdadeira para sua alternativa funcionar — e como você a testaria antes da reunião do conselho?</p></div>':''}${bars(s.round1,'Primeira rodada')}`;
