...
// Fora das rodadas (discussão) o caso continua visível, mas as alternativas são só leitura.
function caseBlock(s,round,votable=true){const chosen=s.mine?.['round'+round];return `<div class="panel"><p class="eyebrow">Caso financeiro</p><h2>${esc(s.case_title)}</h2><p class="case">${esc(s.case_text)}</p></div><div class="options">${s.options.map((o,i)=>{const body=`<span class="letter">${esc(o.letter)}</span><span><h3>${esc(o.title)}</h3><p>${esc(o.text)}</p><span class="tradeoffs"><span class="gain">Potencial: ${esc(o.gain)}</span><span class="cost">Custo: ${esc(o.cost)}</span></span></span>`;return votable?`<button type="button" class="option ${chosen===i?'selected':''}" aria-pressed="${chosen===i}" data-choice="${i}">${body}</button>`:`<article class="option static ${chosen===i?'selected':''}">${body}</article>`}).join('')}</div>`}
