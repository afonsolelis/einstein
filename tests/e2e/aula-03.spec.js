...
test('cronograma oferece download direto da base da aula 3', async ({ page }) => {
  ...
  await expect(link).toHaveAttribute('href', 'materiais/aula-03/dados/hospital_patients_real_world.csv');
...
test('base publicada mantém conteúdo e assinatura esperados', async ({ request }) => {
  const response = await request.get('/materiais/aula-03/dados/hospital_patients_real_world.csv');
...
test('aula 3 percorre prompts, controles e navegação cruzada', async ({ page }) => {
  const errors = watchPage(page);
...
test('slides e material da aula 3 não geram overflow horizontal', async ({ page }) => {
